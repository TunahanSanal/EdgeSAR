...
% -- Tables and Floats --
\usepackage{booktabs,multirow,array,tabularx,xltabular,makecell}
\usepackage{float}
...
\begin{itemize}
    \item Gerçek MSTAR zemin kargaşası ve hedef dağılımı: $\sigma / \mu \in [0.71, 0.94]$ aralığında olup Rayleigh/\allowbreak Gamma dağılımlı doğal radar speckle karakteristiği sergiler.
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{4.5cm} >{\centering\arraybackslash}p{3.4cm} >{\centering\arraybackslash}p{4.2cm} >{\centering\arraybackslash}X}
\toprule
\textbf{Değerlendirme Boyutu} & \makecell{\textbf{Sentetik}\\\textbf{Benzetim}} & \makecell{\textbf{Gerçek MSTAR}\\\textbf{(17° $\to$ 15° SOC)}} & \makecell{\textbf{Gerçek MSTAR}\\\textbf{5-Fold CV}} \\
\midrule
Veri Kaynağı & Nokta Saçıcı Simülatörü & Sandia X-Band MSTAR & Sandia X-Band MSTAR \\
Toplam Hedef Örneği & 675 çip (Eğitim: 600, Test: 75) & 1,285 çip (Eğitim: 698, Test: 587) & 1,285 çip (5 katman) \\
...
\textbf{Not (Tekrarlanabilirlik ve Checkpoint Kararlılığı):} Tablo \ref{tab:mstar_vs_synthetic}'teki \%63.71 doğruluk, depoda commit'li \path{best_mstar_model.pth} ağırlıklarına aittir ve \url{scripts/evaluate_mstar_comprehensive.py} değerlendirme betiğiyle deterministik olarak üretilebilir. Küçük veri setinde (698 örnek) sıfırdan yapılan taze eğitimler (\texttt{--retrain}) rastgele seed varyansı nedeniyle farklı ve daha yüksek sonuçlar (\%82.96) verebilmektedir; bilimsel tutarlılık adına rapordaki tüm analizler sabit checkpoint üzerinden yürütülmüştür.
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.2cm} ccccc >{\raggedright\arraybackslash}X}
\toprule
\textbf{Yapılandırma} & \textbf{Ağırlıklar} & \textbf{Doğruluk} & \makecell{\textbf{Makro}\\\textbf{F1}} & \makecell{\textbf{BMP-2}\\\textbf{Recall}} & \makecell{\textbf{BTR-70}\\\textbf{Recall}} & \textbf{Kritik Bulgular ve Trade-Off} \\
\midrule
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.2cm} ccccc >{\raggedright\arraybackslash}X}
\toprule
\textbf{Kriter} & \textbf{Persentil} & \makecell{\textbf{Eşik}\\\textbf{($\tau$)}} & \makecell{\textbf{Bilinen}\\\textbf{TPR}} & \makecell{\textbf{OOD}\\\textbf{FPR}} & \makecell{\textbf{OOD}\\\textbf{Reddi}} & \textbf{Operasyonel Değerlendirme} \\
\midrule
\textbf{TPR \%95 Koruma} & \%5.0 & 5.4572 & \textbf{\%94.89} & \%85.04 & \textbf{\%14.96} & Bilinen hedefler korunur, OOD reddi zayıftır. \\
\addlinespace[2pt]
\textbf{TPR \%90 Koruma} & \%10.0 & 5.4620 & \textbf{\%89.95} & \%74.76 & \textbf{\%25.24} & Dengeli askeri eşikleme seviyesi. \\
\addlinespace[2pt]
\textbf{TPR \%80 Koruma} & \%20.0 & 5.4745 & \textbf{\%79.90} & \%59.14 & \textbf{\%40.86} & \%40 OOD reddi elde edilir (koruma düşer). \\
\bottomrule
\end{tabularx}
\end{table}
...
Sıfır dinamik bellek kuralının regrese olmasını engellemek için geliştirilen ve \texttt{test\_}\allowbreak\texttt{regression.py} dosyasında koşan AST tarayıcısı mutasyon testine tabi tutulmuştur:
\begin{enumerate}
...
CI/CD Doğrulama & \texttt{v1.1-ci-fixed} & GitHub Actions canlı pipeline'ı iki gerçek cross-platform hatayı (eksik Python tip importu, Linux GNU ld bağlayıcı sıra kuralı) yakaladı; ikisi de şeffaf şekilde düzeltildi, tüm işler (23 test, 13 Unity, Cppcheck, WCET derlemesi) yeşile döndü. & Doğrulandı, canlı. \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Sürekli Entegrasyon (CI/CD) ile Ek Doğrulama:} Proje GitHub'a taşındıktan sonra, kurulan
GitHub Actions iş hattı ilk çalıştırmasında iki gerçek hata tespit etmiştir: (1) \texttt{train.py}
içinde geliştirme ortamında fark edilmeyen, yalnızca temiz bir Python yorumlayıcısında ortaya çıkan
eksik bir \texttt{typing.Optional} içe aktarımı; (2) Windows/MinGW ortamında sorun çıkarmayan ancak
Linux'un GNU \texttt{ld} bağlayıcısının tek-yönlü sembol çözümleme kuralı nedeniyle \texttt{-lm}
matematik kütüphanesi bayrağının komut sırasına duyarlı olduğu bir gömülü C derleme hatası. Her iki
sorun da tespit edildiği gün içinde düzeltilmiş, düzeltmeler commit geçmişine işlenmiş ve pipeline
\texttt{v1.1-ci-fixed} etiketiyle tamamen yeşile dönmüştür. Bu olay, raporun ana ilkesini
doğrulamaktadır: iddialar tek bir geliştirme ortamına değil, bağımsız ve tekrarlanabilir otomatik
doğrulamaya dayanmalıdır.
